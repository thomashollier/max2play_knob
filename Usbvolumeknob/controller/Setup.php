...
class Exampleclass_Setup extends Service
{
    ...
    public function __construct()
    {
        ...
        $this->pluginname = _('Usbvolumeknob');
		...
		if ($ajax == 0) {
            ...
            if ($this->getProgressWithAjax($progressfile, $create = 1, $reloadWhenFinished = 1, $lastlines = 0, $message = _t("Install Something Started"))) {
                $shellanswer = $this->writeDynamicScript(array(
                    $this->scriptPath . "install.sh " . $_REQUEST['server-ip'] . " " . $_REQUEST['player-mac'] . " \"" . $_REQUEST['hid-name'] . "\" >> " . $progressfile . " 2>&1 &"
...
$Usbvolumeknob = new Exampleclass_Setup();
