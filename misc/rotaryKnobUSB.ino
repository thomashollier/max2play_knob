...
void loop()
{
  ...
  if (enc_action > 0) {
    ...
    TrinketKeyboard.pressKey(KEYCODE_MOD_LEFT_SHIFT, KEYCODE_U);
    TrinketKeyboard.pressKey(KEYCODE_MOD_LEFT_SHIFT, 0);
    TrinketKeyboard.pressKey(0, 0);
  }
  ...
  if (bit_is_set(TRINKET_PINx, PIN_ENCODER_SWITCH)) 
  {
    ...
    {
      TrinketKeyboard.pressKey(KEYCODE_MOD_LEFT_SHIFT, KEYCODE_P);
      TrinketKeyboard.pressKey(0, 0);
  ...
  else
  {
    ...
    TrinketKeyboard.poll(); // check if USB needs anything done
}
